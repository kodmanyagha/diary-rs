...
pub fn TimerDisplay(props: &Props) -> Html {
    ...
    let get_session_display: String = {
        ...
        if is_expired {
            format_time(*props_copy.timer_duration)
        } else {
            format_time(*props_copy.session_length - *props_copy.timer_duration)
        }
    };
    ...
    html!(
        <div class={classes!("flex", "flex-col", "space-y-2", "items-center")}>
            ...
            <p class={classes!("text-5xl")}>{session_state_display}</p>
        </div>
    )
}
